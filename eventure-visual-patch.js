...
const polishDiscover=()=>{
  const cards=document.querySelectorAll('#discoverList .card');
  cards.forEach(card=>{
    const title=card.querySelector('h2');
    if(!title||!title.textContent.toLowerCase().includes('winter trail'))return;
    const photo=card.querySelector('.photo');
    if(photo)photo.classList.add('eventureWinterCard');
  });
};
const polishHome=()=>{
  const home=document.getElementById('home');
  if(!home)return;
  const highlight=home.querySelector('.highlight');
  if(highlight&&!home.querySelector('.eventureHomeVisual')){
    const img=document.createElement('img');
    img.src=HERO;
    img.alt='Winter Trail Pra Loup — du 11 au 13 décembre 2026';
    img.className='eventureHomeVisual';
    highlight.parentNode.insertBefore(img,highlight);
  }
};
const polish=()=>{
  scheduled=false;
